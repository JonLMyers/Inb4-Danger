...
def individual_daily_message_count(timestamps, name):  
    eval_date = {"Year": 0, "Month": 0, "Day": 0, "Count": 0}
    count_structure = []
    is_first = True
    for timestamp in timestamps:
        if timestamp['name'] == name:
            count = int(eval_date["Count"])
            year = timestamp['timestamp'][:4]
            month = timestamp['timestamp'][5:7]
            day = timestamp['timestamp'][8:10]
            ...
            if is_first:
                ...
                count_structure.append({"Name": name, "Year": eval_date["Year"], "Month": eval_date["Month"], "Day": eval_date["Day"], "Count": eval_date["Count"]})
                eval_date["Year"] = year
                eval_date["Month"] = month
                eval_date["Day"] = day
                eval_date["Count"] = 1
